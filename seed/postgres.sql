-- dbctl sample schema (Postgres): users, credits, usage, logs
-- Connection: dbctl pg ...   (postgres on localhost:5433)

CREATE TABLE IF NOT EXISTS users (
    id           SERIAL PRIMARY KEY,
    name         TEXT NOT NULL UNIQUE,
    credits_daily  INTEGER NOT NULL DEFAULT 100,
    credits_yearly INTEGER NOT NULL DEFAULT 36500,
...
INSERT INTO credits (user_id, period, limit_value, consumed)
SELECT id, 'Daily', credits_daily, 0 FROM users WHERE name = 'alice'
ON CONFLICT DO NOTHING;
INSERT INTO credits (user_id, period, limit_value, consumed)
SELECT id, 'Yearly', credits_yearly, 0 FROM users WHERE name = 'alice'
ON CONFLICT DO NOTHING;
...
UNION ALL
SELECT id, 'WARN',  'credits near limit' FROM users WHERE name = 'bob'
UNION ALL
